...
bool FSToolCompPose::handleDoubleClick(S32 x, S32 y, MASK mask)
{
        ...
        mManip->getHighlightedPart() == LLManip::LL_NO_PART)
    {
        ...
        mPoser = dynamic_cast<BDFloaterPoser*>(LLFloaterReg::showInstance("poser"));
        return true;
    }
    else
    {
...
//BD - Visual Poser Translation Tool
// If you want a standard static instance approach:
BDToolCompPoseTranslate* BDToolCompPoseTranslate::getInstance()
{
    // Meyers singleton pattern
    static BDToolCompPoseTranslate instance;
    return &instance;
}

//-----------------------------------
// Constructor
BDToolCompPoseTranslate::BDToolCompPoseTranslate()
    : LLToolComposite(std::string("PoseTranslate"))
{
    // Create a joint manipulator
    mManip = new BDManipTranslateJoint(this);
...
//-----------------------------------
// Handle Hover
bool BDToolCompPoseTranslate::handleHover(S32 x, S32 y, MASK mask)
{
    // If the current subtool hasn't captured the mouse,
    // switch to your manip subtool (like LLToolCompRotate).
    if (!mCur->hasMouseCapture())
...
//-----------------------------------
// Handle MouseDown
bool BDToolCompPoseTranslate::handleMouseDown(S32 x, S32 y, MASK mask)
{
    mMouseDown = true;
...
//-----------------------------------
// The pickCallback
void BDToolCompPoseTranslate::pickCallback(const LLPickInfo& pick_info)
{
    BDToolCompPoseTranslate* self = BDToolCompPoseTranslate::getInstance();
    BDManipTranslateJoint* manip = self->mManip;
    ...
    {
        // If no ring is highlighted, reset interaction or do nothing
        LL_INFOS("BDToolCompPoseTranslate") << "No manipulator arrow selected" << LL_ENDL;
    }
}
...
//-----------------------------------
// Handle MouseUp
bool BDToolCompPoseTranslate::handleMouseUp(S32 x, S32 y, MASK mask)
{
    mMouseDown = false;
    // The base LLToolComposite sets mCur->handleMouseUp(...) 
    // and does other management
...
// getOverrideTool
// If you want SHIFT+CTRL combos to do something else
LLTool* BDToolCompPoseTranslate::getOverrideTool(MASK mask)
{
    // Example from LLToolCompRotate that calls scale if SHIFT+CTRL
    if (mask == (MASK_CONTROL | MASK_SHIFT))
    {
...
//-----------------------------------
// Handle DoubleClick
bool BDToolCompPoseTranslate::handleDoubleClick(S32 x, S32 y, MASK mask)
{
    if (!mManip->getSelection()->isEmpty() &&
        mManip->getHighlightedPart() == LLManip::LL_NO_PART)
    {
...
//-----------------------------------
// render
void BDToolCompPoseTranslate::render()
{
    // Render the current subtool
    mCur->render();
