...
void FSManipRotateJoint::highlightHoverSpheres(S32 mouseX, S32 mouseY)
{
    ...
    for (const auto& entry : getSelectableJoints())
    {
        ...
        LLCachedControl<F32> target_radius(gSavedSettings, "FSManipRotateJointTargetSize", 0.1f);
...
void FSManipRotateJoint::setJoint(LLJoint* joint)
{
    ...
    if (mJoint)
    {
        ...
        {
            mSavedJointRot = mJoint->getRotation();
        }
        else
        {
            mSavedJointRot = mJoint->getTargetRotation();
        }
...
void FSManipRotateJoint::render()
{
    ...
    LLQuaternion active_rotation = joint_world_rotation;
...
void FSManipRotateJoint::renderNameXYZ(const LLVector3 &vec)
{
    ...
    gGL.pushMatrix();
    {
        ...
        gGL.color4f(1.f, 1.f, 1.f, 1.0f);

        imagep->draw(
            (S32)((window_center_x - 160) * display_scale.mV[VX]),
            (S32)((window_center_y + vertical_offset - PAD) * display_scale.mV[VY]),
            (S32)(340 * display_scale.mV[VX]),
            (S32)((PAD * 2 + 5) * display_scale.mV[VY] * 2),
...
bool FSManipRotateJoint::handleMouseDownOnPart(S32 x, S32 y, MASK mask)
{
    ...
    auto* poser = dynamic_cast<BDFloaterPoser*>(LLFloaterReg::getInstance("poser"));
    mSavedJointRotVec = poser->getJointRot(mJoint->getName());
...
bool FSManipRotateJoint::handleMouseUp(S32 x, S32 y, MASK mask)
{
    ...
    if (hasMouseCapture())
    {   
        // Update the UI, by causing it to read back the position of the selected joints and apply those relative to the base rot
        if (poser)
        {
            poser->onJointRecapture();
            //poser->onJointControlsRefresh();
        } 
...
F32 FSManipRotateJoint::dragConstrained(S32 x, S32 y)
{
    ...
    if (mCamEdgeOn)
    {
        ...
        if (gSavedSettings.getBOOL("SnapEnabled")) {
            ...
            if (snap_plane > 0)
            {
                ...
                object_axis = object_axis * mJoint->getWorldRotation();
...
void FSManipRotateJoint::drag(S32 x, S32 y)
{
    ...
    LLQuaternion rot_quat = mSavedJointRot;
    LLMatrix3 rot_mat;
    F32 new_value = 0.0f;
    LLVector4 vec4;
    LLVector3 vec3;
    ...
    {
        new_value = dragConstrained(x, y);
        if (mManipPart == LL_ROT_X)
        {
            new_value = new_value - mSavedJointRotVec[VX];
            vec3.mV[VX] = new_value;
        }
        else if (mManipPart == LL_ROT_Y)
        {
            new_value = new_value - mSavedJointRotVec[VY];
            vec3.mV[VY] = new_value;
        }
        else
        {
            new_value = new_value - mSavedJointRotVec[VZ];
            vec3.mV[VZ] = new_value;
        }
...
void FSManipRotateJoint::renderSnapGuidelines()
{
    ...
    if (LLSelectMgr::getInstance()->getGridMode() == GRID_MODE_REF_OBJECT)
    {
        test_axis = test_axis * ~grid_rotation;
            ...
            for (S32 i = 0; i < 64; i++)
            {
                ...
                if (pass == 1 && render_text && i % 16 == 0)
                {
                    if (world_snap_axis.mV[VX])
                    {
                        if (i == 0)
                        {
                            renderTickText(text_point, LLTrans::getString("Direction_Forward"), LLColor4::white);
                        }
                        else if (i == 16)
                        {
                            if (constraint_axis.mV[VZ] > 0.f)
                            {
                                renderTickText(text_point, LLTrans::getString("Direction_Left"), LLColor4::white);
                            }
                            else
                            {
                                renderTickText(text_point, LLTrans::getString("Direction_Right"), LLColor4::white);
                            }
                        }
                        else if (i == 32)
                        {
                            renderTickText(text_point, LLTrans::getString("Direction_Back"), LLColor4::white);
                        }
                        else
                        {
                            if (constraint_axis.mV[VZ] > 0.f)
