...
LLVector3 LLPipeline::RenderScreenSpaceReflectionIterations;
LLVector3 LLPipeline::RenderScreenSpaceReflectionRayStep;
LLVector3 LLPipeline::RenderScreenSpaceReflectionDistanceBias;
LLVector3 LLPipeline::RenderScreenSpaceReflectionDepthRejectBias;
LLVector3 LLPipeline::RenderScreenSpaceReflectionAdaptiveStepMultiplier;
...
// [/SL:KB]
    ...
    //BD
	if (hdr ||shadow_detail > 0 || ssao || RenderDepthOfField || RenderDeferredBlurLight)
	...
	for (LLCullResult::drawinfo_iterator i = gPipeline.beginRenderMap(type); i != gPipeline.endRenderMap(type); ++i)
	{
		...
		if (params)
		{
			if (params->mVertexBuffer.notNull() && params->mLastModelMatrix)
			{
				...
				params->mVertexBuffer->drawRange(LLRender::TRIANGLE_FAN, params->mStart, params->mEnd, params->mCount, params->mOffset);
		...
		if (params)
		{
			if (params->mVertexBuffer.notNull() && params->mLastModelMatrix)
			{
				...
				{ //not batching textures or batch has only 1 texture -- might need a texture matrix
					...
					{
						params->mTexture->addTextureStats(params->mTexture->getMaxVirtualSize());
