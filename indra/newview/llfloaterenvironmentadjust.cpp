...
void LLFloaterEnvironmentAdjust::onHazeDensityChanged()
{
    if (!mLiveSky)
        return;
    mLiveSky->setHazeDensity((F32)mHazeDensity->getValue().asReal());
    mLiveSky->update();
}
...
void LLFloaterEnvironmentAdjust::onSceneGammaChanged()
{
    if (!mLiveSky)
        return;
    mLiveSky->setGamma((F32)mSceneGamma->getValue().asReal());
    mLiveSky->update();
}
...
void LLFloaterEnvironmentAdjust::onDistanceMultipChanged()
{
    if (!mLiveSky)
        return;
    mLiveSky->setCloudShadow((F32)mDistanceMult->getValue().asReal());
    mLiveSky->update();
}
...
void LLFloaterEnvironmentAdjust::onMoonScaleChanged()
{
	if (!mLiveSky) 
		return;
	mLiveSky->setMoonScale((F32)(mMoonScale->getValue().asReal()));
	mLiveSky->update();
}
