...
LLInventoryPanel* LLPanelMainInventory::setupInventoryPanel()
{
	...
	return mInventoryInboxPanel;
}

const U32 LLPanelMainInventory::getFreshItemCount()
{
	//
	...
	{
		LLFolderViewFolder * inbox_folder = mInventoryInboxPanel->getRootFolder();
...
void LLPanelMainInventory::initListCommandsHandlers()
{
	...
	mTrashButton->setCommitCallback(boost::bind(&LLPanelMainInventory::onTrashButtonClick, this));
...
void LLPanelMainInventory::setUploadCostIfNeeded()
{
	...
	if (mNeedUploadCost)
	{
		...
		if (upload_menu)
		{
			...
			upload_menu->getChild<LLView>("Upload Image")->setLabelArg("[COST]", texture_upload_cost_str);
			upload_menu->getChild<LLView>("Upload Sound")->setLabelArg("[COST]", sound_upload_cost_str);
