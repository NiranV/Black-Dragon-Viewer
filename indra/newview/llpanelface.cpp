		...
		&& objectp->permModify())
	{
		...
		// Texture scale
		{
			...
			if (material_type == MATTYPE_SPECULAR)
			{
				...
				mTexScaleU->setEnabled(editable && specmap_id.notNull() && has_material);
				mTexScaleU->setTentative(LLSD(spec_scale_tentative));
			}
			else if (material_type == MATTYPE_NORMAL)
			{
				mTexScaleU->setValue(norm_scale_s);
				mTexScaleU->setEnabled(editable && normmap_id.notNull() && has_material);
...

		{
			...
			if (material_type == MATTYPE_SPECULAR)
			{
				mTexScaleV->setEnabled(editable && specmap_id.notNull() && has_material);
				mTexScaleV->setValue(spec_scale_t);
				mTexScaleV->setTentative(LLSD(spec_scale_tentative));
			}
			else if (material_type == MATTYPE_NORMAL)
			{
				mTexScaleV->setEnabled(editable && normmap_id.notNull() && has_material);
		...
		// Texture offset
		{
			...
			if (material_type == MATTYPE_SPECULAR)
			{
				...
				mTexOffsetU->setEnabled(editable && specmap_id.notNull() && has_material);
			}
			else if (material_type == MATTYPE_NORMAL)
			{
				mTexOffsetU->setValue(editable ? norm_offset_s : 0.0f);
				mTexOffsetU->setTentative(LLSD(norm_offset_u_tentative));
				mTexOffsetU->setEnabled(editable && normmap_id.notNull() && has_material);
			}
			else
			{
...

		{
			...
			if (material_type == MATTYPE_SPECULAR)
			{
				...
				mTexOffsetV->setEnabled(editable && specmap_id.notNull() && has_material);
			}
			else if (material_type == MATTYPE_NORMAL)
			{
				mTexOffsetV->setValue(editable ? norm_offset_t : 0.0f);
				mTexOffsetV->setTentative(LLSD(norm_offset_v_tentative));
				mTexOffsetV->setEnabled(editable && normmap_id.notNull() && has_material);
			}
			else
			{
		...
		// Texture rotation
		{
			...
			if (material_type == MATTYPE_SPECULAR)
			{
				mTexRot->setEnabled(editable && specmap_id.notNull() && has_material);
				mTexRot->setTentative(LLSD(spec_rot_tentative));
				mTexRot->setValue(editable ? spec_rot_deg : 0.0f);
			}
			else if (material_type == MATTYPE_NORMAL)
			{
				mTexRot->setEnabled(editable && normmap_id.notNull() && has_material);
		...
		// Repeats per meter
		{
			...
			mRepeats->setEnabled(has_material && !identical_planar_texgen && enabled);
