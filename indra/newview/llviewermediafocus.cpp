...
//BD - UI Improvements
bool LLViewerMediaFocus::handleScrollWheel(const LLVector2& texture_coords, S32 clicks_x, S32 clicks_y, MASK mask)
{
    bool retval = false;
    LLViewerMediaImpl* media_impl = getFocusedMediaImpl();
    if (media_impl && media_impl->hasMedia())
    {
        media_impl->scrollWheel(texture_coords, clicks_x, clicks_y, mask);
        retval = true;
    }
    return retval;
}

bool LLViewerMediaFocus::handleScrollWheel(S32 x, S32 y, S32 clicks_x, S32 clicks_y, MASK mask)
{
    bool retval = false;
    LLViewerMediaImpl* media_impl = getFocusedMediaImpl();
    if(media_impl && media_impl->hasMedia())
    {
        media_impl->scrollWheel(x, y, clicks_x, clicks_y, mask);
        retval = true;
    }
    return retval;
}
