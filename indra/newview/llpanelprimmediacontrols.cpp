...
bool LLPanelPrimMediaControls::handleScrollWheel(S32 x, S32 y, S32 clicks, MASK mask)
{
    ...
    if (LLViewerMediaFocus::getInstance()->isHoveringOverFocused())
    {
        ...
        res = LLToolPie::getInstance()->handleScrollWheel(x, y, clicks, mask);
    }

    return res;
}

bool LLPanelPrimMediaControls::handleScrollHWheel(S32 x, S32 y, S32 clicks, MASK mask)
{
    mInactivityTimer.start();
    bool res = false;
    ...
    {
        // either let toolpie handle this or expose mHoverPick.mUVCoords in some way
        res = LLToolPie::getInstance()->handleScrollHWheel(x, y, clicks, mask);
    }
